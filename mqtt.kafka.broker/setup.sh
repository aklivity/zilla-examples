...
helm install zilla-mqtt-kafka-broker $ZILLA_CHART --namespace zilla-mqtt-kafka-broker --create-namespace --wait \
    --values values.yaml \
...
kubectl port-forward --namespace zilla-mqtt-kafka-broker service/zilla-mqtt-kafka-broker 7183 7883 > /tmp/kubectl-zilla.log 2>&1 &
