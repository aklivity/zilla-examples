...
helm upgrade --install zilla-http-kafka-cache $ZILLA_CHART --namespace zilla-http-kafka-cache --create-namespace --wait \
    --values values.yaml \
...
kubectl port-forward --namespace zilla-http-kafka-cache service/zilla-http-kafka-cache 7114 7143 > /tmp/kubectl-zilla.log 2>&1 &
