...
helm upgrade --install zilla-grpc-kafka-fanout $ZILLA_CHART --namespace $NAMESPACE --create-namespace --wait \
    --values values.yaml \
...
helm upgrade --install zilla-grpc-kafka-fanout-kafka chart --namespace $NAMESPACE --create-namespace --wait
...
kubectl port-forward --namespace $NAMESPACE service/zilla-grpc-kafka-fanout 7153 > /tmp/kubectl-zilla.log 2>&1 &
