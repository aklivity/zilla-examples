...
helm upgrade --install zilla-http-filesystem $ZILLA_CHART --namespace $NAMESPACE --create-namespace --wait \
    --values values.yaml \
...
kubectl port-forward --namespace $NAMESPACE service/zilla-http-filesystem 7114 7143 > /tmp/kubectl-zilla.log 2>&1 &
