...
helm upgrade --install zilla-http-kafka-sasl-scram $ZILLA_CHART --namespace $NAMESPACE --create-namespace --wait \
    --values values.yaml \
    --set-file zilla\\.yaml=zilla.yaml \
    --set-file secrets.tls.data.localhost\\.p12=tls/localhost.p12

# Install Kafka and Zookeeper to the Kubernetes cluster with helm and wait for the pods to start up
helm upgrade --install zilla-http-kafka-sasl-scram-kafka chart --namespace $NAMESPACE --create-namespace --wait
...
kubectl port-forward --namespace $NAMESPACE service/zilla-http-kafka-sasl-scram 7114 7143 > /tmp/kubectl-zilla.log 2>&1 &
