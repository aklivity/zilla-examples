...
helm upgrade --install zilla-http-proxy $ZILLA_CHART --namespace $NAMESPACE --create-namespace --wait \
    --values values.yaml \
...
helm upgrade --install zilla-http-proxy-nginx chart --namespace $NAMESPACE --create-namespace --wait
...
kubectl port-forward --namespace $NAMESPACE service/zilla-http-proxy 7143 > /tmp/kubectl-zilla.log 2>&1 &
