...
helm upgrade --install zilla-grpc-kafka-proxy $ZILLA_CHART --namespace $NAMESPACE --create-namespace --wait \
    --values values.yaml \
...
helm upgrade --install zilla-grpc-kafka-proxy-kafka chart --namespace $NAMESPACE --create-namespace --wait --timeout 2m
...
kubectl port-forward --namespace $NAMESPACE service/zilla-grpc-kafka-proxy 7153 > /tmp/kubectl-zilla.log 2>&1 &
