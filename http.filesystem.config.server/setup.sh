...
helm upgrade --install zilla-config-server-config $ZILLA_CHART --namespace zilla-config-server --create-namespace --wait \
...
helm upgrade --install zilla-config-server-http $ZILLA_CHART --namespace zilla-config-server --create-namespace --wait \
...
kubectl run busybox-pod --image=busybox:1.28 --namespace zilla-config-server --rm --restart=Never -i -t -- /bin/sh -c 'until nc -w 2 zilla-config-server-http 7114; do echo . && sleep 5; done' > /dev/null 2>&1
...
kubectl port-forward --namespace zilla-config-server service/zilla-config-server-http 7114 7143 > /tmp/kubectl-zilla-http.log 2>&1 &
