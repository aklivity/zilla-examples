...
helm upgrade --install zilla-grpc-kafka-echo $ZILLA_CHART --namespace $NAMESPACE --create-namespace --wait \
    --values values.yaml \
...
helm upgrade --install zilla-grpc-kafka-echo-kafka chart --namespace $NAMESPACE --create-namespace --wait
...
kubectl port-forward --namespace $NAMESPACE service/zilla-grpc-kafka-echo 7151 7153 > /tmp/kubectl-zilla.log 2>&1 &
