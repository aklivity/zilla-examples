...
VERSION=0.9.46
helm install zilla-http-kafka-sync $ZILLA_CHART --version $VERSION --namespace zilla-http-kafka-sync --create-namespace --wait \
    --values values.yaml \
...
kubectl port-forward --namespace zilla-http-kafka-sync service/zilla-http-kafka-sync 7114 7143 > /tmp/kubectl-zilla.log 2>&1 &
