...
class Topic(ABC):
    ...
    def connect(self):
        self.client = mqtt.Client(self.topic_url, transport='tcp', protocol=mqtt.MQTTv5)
    ...
    def on_message(client, userdata, message):
        ...
        print("user properties received= ",message.properties)

    def on_publish(self, client, userdata, result):
        print(f'[{time.strftime("%H:%M:%S")}] Data published on: {self.topic_url}')

    def on_connect(self, client, _, flags, return_code):
        print("MQTT connected")
        # print(f'Data published on: {flags}')
        # print("MQTT connection flags: %s", flags)
        # print("MQTT connection returned result: %s", mqtt.connack_string(return_code))

    def on_connect_fail(self, client, _, flags, return_code):
...
class TopicAuto(Topic, threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            publish_properties.UserProperty = ("Idempotency-Key", self.topic_url)
            self.client.publish(topic=self.topic_url, payload=json.dumps(payload), qos=0, retain=True, properties=publish_properties)
