...
helm upgrade --install zilla-http-kafka-cache $ZILLA_CHART --namespace $NAMESPACE --create-namespace --wait \
    --values values.yaml \
    --set-file zilla\\.yaml=zilla.yaml \
    --set-file secrets.tls.data.localhost\\.p12=tls/localhost.p12

# Install Kafka to the Kubernetes cluster with helm and wait for the pod to start up
helm upgrade --install zilla-http-kafka-cache-kafka chart --namespace $NAMESPACE --create-namespace --wait
...
kubectl port-forward --namespace $NAMESPACE service/zilla-http-kafka-cache 7114 7143 > /tmp/kubectl-zilla.log 2>&1 &
