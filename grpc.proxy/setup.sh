...
helm upgrade --install zilla-grpc-proxy $ZILLA_CHART --namespace $NAMESPACE --create-namespace --wait \
    --values values.yaml \
...
helm upgrade --install zilla-grpc-proxy-grpc-echo chart --namespace $NAMESPACE --wait --timeout 2m

# Start port forwarding
kubectl port-forward --namespace $NAMESPACE service/zilla-grpc-proxy 7153 > /tmp/kubectl-zilla.log 2>&1 &
